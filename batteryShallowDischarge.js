...
function resetCycle(force, date) {
    ...
    date = date || new Date();
    if (force || !getState()) {
        var state = { cycleMonth: date.getMonth()+1, shallow: getShallow() };
        setState(state);
...
setState({ cycleMonth: new Date().getMonth()+1-1 });
...
setState({ cycleMonth: new Date().getMonth()+1, shallow: 'upper' });
...
setState({ cycleMonth: new Date().getMonth()+1, shallow: 'lower' });
...
// % jump tests
setState({ cycleMonth: new Date().getMonth()+1, shallow: 'lower' }); lastPercentage = 75;
